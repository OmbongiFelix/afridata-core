...
Registered models:
  UserInteraction      — list by user, interaction_type, created_at
  DatasetProxy         — list by title, category, last_synced_at
...
class DatasetAdmin(admin.ModelAdmin):
    list_display = ("title", "category", "last_synced_at")
    list_filter = ("category",)
    readonly_fields = ("title", "category", "last_synced_at")
...
class RecommendationResultAdmin(admin.ModelAdmin):
    ...
    def ranked_items_preview(self, obj):
        """Read-only JSON preview of ranked_dataset_ids."""
        import json
        try:
            return json.dumps(obj.ranked_dataset_ids, indent=2)
        except (TypeError, ValueError):
            return str(obj.ranked_dataset_ids)
